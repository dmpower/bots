...
public class Starter extends AdvancedRobot {
	...
	// allowing a full sweep causes slippage. This allows it to adjust to keep
	// target centered
...
// this.gunRack.add(new HeadOnSlicerGun(this.targetManager, this));
	...
	/*
	 * This method receives a ScannedRobotEvent and uses that information to
	 * create a new wave and place it in our log. Basically we're going to take
	 * all the information we'll need to know later to figure out where to move
	 * to and store it in one object so we can use it easily later.
	 */
	public void logMovementWave(TargetBot e, double energyChange) {
		...
		w.startTime = getTime();
		// The absolute bearing from the enemy to us can be found by adding Pi
		// to our absolute bearing.
		w.angle = Utils.normalRelativeAngle(absBearing + Math.PI);
		/*
		 * Our lateral velocity, used to calculate where a bullet fired with
		 * linear targeting would be. Note that the speed has already been
	...
	/*
	 * This method looks at all the directions we could go, then rates them
	 * based on how close they will take us to simulated bullets fired with both
	 * linear and head-on targeting generated by the waves we have logged. It is
	...
	private void doMove() {
		if (hasTarget()) {
			...
			for (double moveAngle = 0; moveAngle < Math.PI * 2; moveAngle += Math.PI / 16D) {
				...
				// Movepoint is position we would be at if we were to move one
				// robot-length in the given direction.
				Point movePoint = BotTools.project(new Point(getX(), getY()), 36, moveAngle);

				/*
				 * This loop will iterate through each wave and add a risk for
				 * the simulated bullets on each one to the total risk for this
				 * angle.
				 */
				for (int i = 0; i < this.moveWaves.size(); i++) {
					w = this.moveWaves.get(i);

					// This part will remove waves that have passed our robot,
					// so we no longer keep taking into account old ones
					if (new Point(getX(), getY()).distance(w.origin) < (getTime() - w.startTime) * w.speed + w.speed) {
						this.moveWaves.remove(w);
					} else {
						/*
						 * This adds two risks for each wave: one based on the
						 * distance from where a head-on targeting bullet would
						 * be, and one for where a linear targeting bullet would
						 * be.
						 */
						rating += 1D / Math.pow(
								movePoint.distance(BotTools.project(w.origin, movePoint.distance(w.origin), w.angle)),
								2);
						rating += 1D / Math.pow(
								movePoint.distance(
										BotTools.project(w.origin, movePoint.distance(w.origin), w.angle + w.latVel)),
								2);
						rating += Math.abs(BotTools.convertToPoint(this).distance(movePoint) - 500);
					}
				}
				// This adds a risk associated with being too close to the other
				// robot if there are no waves.
				if (this.moveWaves.size() == 0) {
					rating = 1D / Math.pow(movePoint.distance(enemyLocation), 2);
				}
				// This part tells us to go in the direction if it is better
				...
				if (rating < bestRating
					 ...
					 * to choose the best direction for moving to a point.
					 */
					int pointDir;
					setAhead(1000 * (pointDir = (Math.abs(moveAngle - getHeadingRadians()) < Math.PI / 2 ? 1 : -1)));
					setTurnRightRadians(Utils
							.normalRelativeAngle(moveAngle + (pointDir == -1 ? Math.PI : 0) - getHeadingRadians()));
				}
			}// end for
		}
	}
	...
	public void onHitRobot(HitRobotEvent event) {
		this.out.println(
				"Turn: " + getTime() + " - hit " + event.getName() + " and they have " + event.getEnergy() + " left.");
	...
	private void doRadar() {
		...
		// this.out.println("Rader turn: " + currentTime + " remaining turn: " +
		...
		// this.out.println("Rader remaining turn: " +
		// getRadarTurnRemainingRadians());

		if (Math.abs(getRadarTurnRemainingRadians()) < RADAR_TOLLARENCE) {
			// this.out.println("Radar completed turn");
			// basically if there is only a little bit of turn remaining, we
			// want to turn
			// the radar back anyways. Otherwise the radar stops mid turn and we
			// lose most
			// of the radar's time sitting still.

			// note: we use POSITIVE_INFINITY instead of RADAR_HALF_SWEEP
			// because
			// if there is a slip we will keep turning until we find another
			// robot.
	...
	private void doGun() {
		...
		for (Gun gun : this.gunRack) {
			...
			// this.out.println(stats.toString() + " for " +
	...
	private int compareGunStats(Gun gun1, Gun gun2) {
		...
		// these two checks allow the gun to get some time in after initial
		// firings.
		...
		if (0 == results && Math.abs(averageDiff) > 0.01) { // both have had
			// chances, which is
			// better?
	...
	@Override
	public long getTime() {
		// This is an important override. GetTime is the most used call and we
		// have a set number of calls
	...
	public void onPaint(Graphics2D g) {
		...
		if (PAINT_MOVEMENT) {
			for (int i = 0; i < this.moveWaves.size(); i++) {
				...
				g.drawArc((int) (w.origin.x - radius), (int) (w.origin.y - radius), (int) radius * 2, (int) radius * 2,
						(int) sArc, (int) eArc);
				radius = 2d;
		...
		if (PAINT_BOT_RAYS) {
			...
			// double
			// absBearing=currentTarget().getBearingRadians()+getHeadingRadians();
			// Point targetPoint = BotTools.project(selfPoint, RAY, absBearing);
			// g.setColor(Color.yellow);
			// g.drawLine((int)targetPoint.getX(), (int)targetPoint.getY(),
			// (int)getX(), (int)getY());
			// }
		...
		if (PAINT_CURRENT_TARGET) {
			if (hasTarget()) {
				...
				g.drawRect((int) targetPoint.getX() - botHalfWidth, (int) targetPoint.getY() - botHalfWidth, botWidth,
						botWidth);
			}
		}
	}

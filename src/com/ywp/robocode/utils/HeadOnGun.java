...
public class HeadOnGun implements Gun {
	...
	public void update() {
		...
		// I hate doing this second pass, but the loop above will not allow me
		// to delete it at the same time.
		for (Entry<String, Vector<Bullet>> target : this.bullets.entrySet()) {
	...
	public void onPaint(Graphics2D g) {
		// note, this seems to run before all other events, so it is one turn
		// behind with what it is drawing.
		if (this.lastTarget != null) {
			...
			g.drawLine((int) this.owningBot.getX(), (int) this.owningBot.getY(), (int) rayPoint.getX(),
					(int) rayPoint.getY());
		}
	}
